import React, { useEffect, useState } from 'react';
import './Sidebar.css'; // Import sidebar styles
import Home from '../home/Home'; // Import Home component
import About from '../about/About'; // Import About component
import Skills from '../skills/Skills'; // Import Skills component
import Project from '../project/Project'; // Import Project component
import Contact from '../contact/Contact'; // Import Contact component


const Sidebar = ({ setSelectedButton, selectedButton }) => {


  const handleButtonClick = (index) => {
    setSelectedButton(index);
  };

  useEffect(() => {
    handleButtonClick(0);
  }, []);

  return (
    <div className="sidebar">
      <div className="logoContainer">
        <img src="https://scontent.fsgn5-10.fna.fbcdn.net/v/t39.30808-6/387145197_847028253768419_5809396859964767534_n.jpg?_nc_cat=107&ccb=1-7&_nc_sid=5f2048&_nc_ohc=dyqC_l9eiv8Q7kNvgFPhfoN&_nc_ht=scontent.fsgn5-10.fna&oh=00_AYDnOZ5Szeeo0GY_J2Vb6C_xiFd2T1PW9JWazM0NWcBCzg&oe=665A4817" alt="Logo" />
      </div>

      <div className="contact2Group">
        <a href="https://www.facebook.com/taovietduc"><img src="https://www.facebook.com/images/fb_icon_325x325.png" alt="Logo" /></a>
        <a href="https://www.instagram.com/taovietduc_/"><img src="https://upload.wikimedia.org/wikipedia/commons/thumb/a/a5/Instagram_icon.png/2048px-Instagram_icon.png" alt="Logo" /></a>
        <a href="https://www.tiktok.com/@taovietduc_"><img src="https://freepngtransparent.com/wp-content/uploads/2023/03/tiktok-png-104.png" alt="Logo" /></a>
      </div>
      <div className="buttonGroup">
        {["HOME", "ABOUT", "SKILLs", "PROJECT", "CONTACT"].map((buttonName, index) => (
          <button
            key={index}
            onClick={() => handleButtonClick(index)}
            className={selectedButton === index ? 'selected' : ''}
          >
            {buttonName}
          </button>
        ))}
      </div>

      <div className="contactGroup">
        <a href="https://github.com/taovietduc"><img src="data:image/jpeg;base64," alt="Logo" /></a>
      </div>

    </div>
  );
};

export default Sidebar;
